...
from Measures import get_snr, get_rmse # Evaluation metrics
...
n_tot = 128        # Total number of bits available per window
...
R_unused = np.zeros((nb_signal, nb_phase, nb_w))  # Unused bits
...
for id_signal in range(nb_signal):
    ...
    for id_phase in range(nb_phase):
        ...
        for w in range(nb_w):
            ...
            nb_vect = 1 # Number of vectors to add at each iteration (the higher nb_vect, the more the computational cost decreases)
            
            # Iterative search for significant vectors
            while True:
                # Select the best vectors to approximate the residual signal
                L_i, gamma_i = O.best_vect(r, D, "MSE",0, nb_vect)
                ...
                # If the total number of bits exceeds the rate constraint, stop the coding process
                if len(code_L) + len(L_test) * 3 > n_tot:
                    break
                ...
                for n_gamma in range(2, 2**nb_n_gamma):
                    ...
                    RMSE_test = get_rmse(x_n, x_rec_test) * 2**kx
                    SNR_test = get_snr(x_n, x_rec_test)
                    
                    # Compute the total number of bits used
                    R_real = len(code_gamma) + len(code_L)
           
                    # If the total bits used (including header) is within the allowable limit:
                    if R_real + R_h[id_signal][id_phase][w] <= n_tot:
                        ...
                        if RMSE_test < RMSE_min:
                            ...
                            R_unused[id_signal][id_phase][w] = n_tot - R_min
                            x_rec[id_signal][id_phase][w * N:w * N + N] = x_rec_test * 2**kx
                            
                    else:
                        # Break out of the loop if the bit budget is exceeded
                        break
...
for id_signal in range(nb_signal):
    ...
    for id_phase in range(nb_phase):
        ...
        plt.bar([i for i in range(nb_w)],R_unused[id_signal][id_phase], width = largeur_barre,bottom =R_pos[id_signal][id_phase]+R_gamma[id_signal][id_phase]+R_h[id_signal][id_phase],color='y',label="R_unused")
        plt.xlabel('index w')
        plt.ylabel('R (bits)')
        plt.legend()
