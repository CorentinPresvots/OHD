...
class Dico_Cos_Sin_Dirac():
    ...
    def Creat_dico(self, N, f_max):
        ...
        # Initialize a constant basis vector
        CST = np.ones((N, 1)) / np.sqrt(N)
        ...
        for raw in range(0, N):
            for col in range(0, nb_vect_freq):
                ...
                F[raw, 2 * col] = np.sqrt(2 / N) * np.cos((np.pi / N) * (raw + 1 / 2) * ((col + 1)) / self.f_factor)
                # Sine basis vectors
        ...
        D = np.concatenate((CST, F, I), axis=1)
        return D
...
if __name__ == "__main__":
    ...
    f_max = 300     # Maximum frequency in the dictionary (Hz)
    ...
    for j in range(1, int((size_D - N) / 2) + 1):
        fig = plt.figure(figsize=(8, 4), dpi=70)
